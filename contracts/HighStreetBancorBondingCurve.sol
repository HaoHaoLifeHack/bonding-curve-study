...
import "./dependencies/SafeMathUpgradeable.sol";
import "./dependencies/Power.sol"; // Efficient power function.
...
*/
contract HighStreetBancorBondingCurve is Power {
  using SafeMathUpgradeable for uint256;
  uint32 private constant MAX_RESERVE_RATIO = 1000000; // 100%, 精度為6位小數
  ...
  */
  // 目前實作只能透過調整 _reserveBalance、_supply、_reserveRatio 來間接影響初始價格
  function calculatePriceForNTokens(
    uint32 _supply,          // 當前代幣總供應量
    uint256 _reserveBalance, // 儲備金餘額
    uint32 _reserveRatio,    // 儲備比率
    uint32 _amount          // 想要購買的代幣數量
  ) public view returns (uint256)
  {
    ...
    // 計算 (baseN/supply)^(MAX_RESERVE_RATIO/_reserveRatio)
    (result, precision) = power(
      baseN, supply, MAX_RESERVE_RATIO, _reserveRatio
    );
    uint256 temp =  _reserveBalance.mul(result) >> precision; // 位移運算（>> precision）來處理小數點
